...
- `POST /api/profile` — name + light/soil/room + optional pot (size/material/drainage) → the 6 profile fields + mist/clean cadence + rationale, applied on every add unless the user hand-edited a field. Baseline includes the pot but is soil/season-agnostic on purpose; the app still applies its soil + seasonal multipliers. **The AI only sets the starting guess — the still-wet/too-busy learning remains the authority and pot fields never feed the learning.**
- `POST /api/diagnose` — photo + the plant's real watering evidence (days since watered, still_wet/too_busy events, season, pot, carnivore) → verdict (over/under/ok/unclear), observations, likely causes, actions. "Check plant health" on Plant Detail (shown only when the server is up); result can be saved to the journal. 20/day per user.
...
- **AI / plant profiles:** When adding a plant, the 6-field profile (watering days, moisture, feed-every-N, fert type, carnivore, water source) is resolved **local-first**:
  ...
  3. **Manual defaults** if the API is also unreachable.
  - **Why local-first:** the metered Anthropic API was the only thing costing money (~$0.004/add). The local DB was generated by Claude Code on the subscription (not the metered key), so known plants now cost $0. To extend coverage, add rows to `LOCAL_PROFILES`/`LOCAL_PROFILE_PATTERNS` (ask Claude Code to generate them — never needs the API). Successful API responses are still cached in `profileCache`; local matches are not written to the cloud cache.
...
- **Profile resolution:** local DB first (free/offline) → Claude API (unknown plants only) → manual entry if API unreachable; failed API lookups are never cached
...
- **`public/{uid}/summary` (per-user, world-readable):** a tiny **names-free** doc the nightly "Plantaroo Watering" Notion routine reads. Written by `writeWateringSummary()` (in `index.html` on render, called from app startup/water/skip in `app/src/lib/wateringSummary.ts`), only when contents change. Fields: `totalPlants`, `neverWateredCount`, `dueDates[]` (computed next-due `YYYY-MM-DD` per watered plant — anonymous, absolute), `newestWateringDaysAgo`, `updatedAt`. **Privacy: only counts + dates — never plant names/notes/rooms/photos.** Per-user isolation via UID path enforced by `firestore.rules` (world read, owner write only). Exists because cloud routines can't auth as Jamal to read `users/{uid}/**`. iOS app publishes on app open and after watering/skipping (OTA deployed 2026-06-12). See `Claude Agent/agents/plantaroo-watering.md`.

### Sharing schema (co-owners + plant-sitter "away mode")
Two capability-token flows, both built on the world-readable `public/{uid}/**` pattern (no Cloud Functions, ships OTA). Code lives in `app/src/lib/sharing.ts`; web pages in `web/sitter/` and `web/join/` (deploy to **jamasha.com/plantaroo/**).
- **Co-owner (Planta "Family"):** owner publishes `public/{ownerUid}/invites/{token}` → invitee (own account) stamps `acceptedBy`/`status` → owner's app finalizes by writing `users/{ownerUid}/members/{memberUid}` (`{role:'coowner', name, addedAt}`). `firestore.rules` grants a member full read/write to the owner's subtree via the `isMember(ownerUid)` helper. The viewer's app loads the owner's data via **`_activeOwnerUid`** in `app/src/data/db.ts` (defaults to the signed-in uid; `switchAccount()` changes it and persists it; `onSnapshot` live-sync attaches only while a *shared* account is active). Auth-bound work (account deletion, the names-free summary) always uses the signed-in uid, never the active owner — `writeWateringSummary()` and `drainGuestEvents()` early-return when `!isViewingOwnAccount()`.
- **Plant-sitter / away mode:** owner publishes `public/{ownerUid}/shares/{token}` (`{ownerName, start, end, plants:[{id,name,room,photo,dueDates[]}], status}`) computed by `getDueDatesInWindow()` (`app/src/logic/schedule.ts`). The sitter opens `jamasha.com/plantaroo/sitter/?u=&t=` (world-read, anon-auth, no app) and check-offs land in `.../shares/{token}/events/{plantId-ymd}`. The owner's app **drains** that inbox on open (`drainGuestEvents()`) and replays each as a real `waterPlant()` so history + learning stay correct (idempotent via applied-id set in AsyncStorage). Away mode is own-account only.
- Token = unguessable capability; shares/invites are revocable; `get` is public but `list` is owner-only (rules) so collections can't be enumerated. Deep links use the `plantaroo://join` custom scheme with a paste-the-link fallback in Settings → Sharing.
- **Deploy:** `firebase deploy --only firestore:rules` + deploy `web/` to jamasha.com (Netlify) + `npx eas update` for the app JS — **no native build** (web link instead of `expo-print`).
